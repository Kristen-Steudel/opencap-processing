...
_BP = os.path.dirname(_SP)
_ROOT = os.path.dirname(_BP)
for _p in (_ROOT, _BP, os.path.join(_SP, 'batch_configs')):
...
if os.path.exists(session_id):
    ...
    if 'opensimdata' in lower_parts:
        ...
        if session_root is None:
            # Bilevel results layout: filtered kinematics under
            # <results>/Cleaned_Kinematics with no OpenSimData parent.
            if os.path.basename(session_id) == 'Cleaned_Kinematics':
                session_root = os.path.dirname(session_id)
            elif os.path.isdir(session_id):
                session_root = session_id
            else:
                raise FileNotFoundError(
                    f"Could not find session root for path: {session_id}")
    ...
    # Always include Cleaned_Kinematics so the filtered .mot is visible when
    # the raw OpenSimData/Kinematics folder also exists (e.g. OpenCap data).
    mot_files = sorted(
        glob.glob(os.path.join(session_root, 'OpenSimData', 'Kinematics', '*.mot')) +
        glob.glob(os.path.join(session_root, 'Cleaned_Kinematics', '*.mot'))
    )
    if not mot_files:
    ...
    # Find model (.osim).
    # If the config provides an explicit 'model_path', use it directly.
    # Otherwise fall back to the standard OpenPose variant folder glob.
    _explicit_model = paths.get('model_path')
    if _explicit_model:
        if not os.path.isfile(_explicit_model):
            raise FileNotFoundError(f"Model file not found: {_explicit_model}")
        model_path = _explicit_model
    else:
