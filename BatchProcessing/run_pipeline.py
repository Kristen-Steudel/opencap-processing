"""
BatchProcessing/run_pipeline.py

Runs the 8-step Football pipeline using a config from this folder or
batch_configs/.  Used by batch_pipeline.py for per-subject processing.

Usage:
    python run_pipeline.py
    python run_pipeline.py --steps 1 3 4
    python run_pipeline.py --config pipeline_config_March_2_ID5_S7_sprint
...
def main():
    parser = argparse.ArgumentParser(
        description='Run the batch Football processing pipeline.')
    parser.add_argument('--steps', nargs='+', metavar='N',
                        help='Step numbers to run (default: all).')
    parser.add_argument('--list', action='store_true',
                        help='List available steps and exit')
    parser.add_argument(
        '--config', default='pipeline_config', metavar='MODULE',
        help='Config module name (default: pipeline_config). '
             'Generated configs live in batch_configs/.')
    ...
    sys.path.insert(0, os.path.join(SCRIPT_DIR, 'batch_configs'))
