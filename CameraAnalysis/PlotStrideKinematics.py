...
# Unsided coordinates whose sign must be flipped when plotting left-leg strides
# so they align with the right-leg NordSprint reference convention.
# (pelvis_list, pelvis_rotation, lumbar_rotation are laterally antisymmetric.)
LEFT_FLIP_COLS = {'pelvis_list', 'pelvis_rotation', 'lumbar_rotation', 'lumbar_bending'}
...
def make_page(page_cols, strides, side_label, colormap, page_num, total_pages,
    ...
    For left-leg strides, laterally antisymmetric unsided coordinates
    (pelvis_list, pelvis_rotation, lumbar_rotation) are negated so they
    compare correctly against the right-leg NordSprint reference.
    """
    ...
    for idx, col in enumerate(page_cols):
        ...
        # Determine whether to negate this column for left-leg strides.
        # Only unsided columns in LEFT_FLIP_COLS are flipped; sided columns
        # already have correct sign because we map ipsi→_r / contra→_l above.
        flip = (side_suffix == '_l') and (col in LEFT_FLIP_COLS)
        ...
        if lit_key and lit_key in lit_curves:
            ...
            sd_label   = f'NordSprint ± 1 SD' \
                         if (not lit_legend_added) else None
            # For left strides, negate laterally antisymmetric literature curves
            # so they match the experimental left-leg sign convention.
            ax.plot(GC_PCT, sign * lc['mean'],
                    color='black', linewidth=2.0, linestyle='-',
                    label=mean_label, zorder=3)
            ax.plot(GC_PCT, sign * (lc['mean'] + lc['std']),
        ...
        # ---- experimental strides (always plotted as-is) ----
